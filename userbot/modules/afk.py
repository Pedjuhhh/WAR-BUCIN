...
async def set_afk(afk_e):
    ...
    if string:
        AFKREASON = string
        await afk_e.edit(f"🖕GA USAH SOKAB🖕\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} BUSY\n┣ ᴀʟᴀꜱᴀɴ : {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
    else:
        await afk_e.edit("🖕BUSY🖕\n╭╼═══╾╼═══╾╼═══╾ \n┣ AFK \n╰╼═══╾╼═══╾")
    if user.last_name:
...
async def type_afk_is_not_true(notafk):
    ...
    user = await bot.get_me()  # pylint:disable=E0602
    last = user.last_name
    if last and last.endswith("🔥GHOSTING🔥"):
        last1 = last[:-12]
    else:
        last1 = ""
    ...
    if mention.message.mentioned and not (await mention.get_sender()).bot:
        if ISAFK:
            ...
            if mention.sender_id not in USERS:
                if AFKREASON:
                    await mention.reply(f"🖕GA USAH SOKAB🖕\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} BUSY\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ LAGI: {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
                else:
    ...
    if sender.is_private and sender.sender_id != 777000 and not (
        ...
        if apprv and ISAFK:
            ...
            if sender.sender_id not in USERS:
                if AFKREASON:
                    await sender.reply(f"🖕GA USAH SOKAB🖕\n╭╼═══╾╼═══╾╼═══╾╼═══╾ \n┣ {ALIVE_NAME} BUSY\n┣ ꜱᴇᴊᴀᴋ: {afk_since}\n┣ ᴀʟᴀꜱᴀɴ: {AFKREASON}\n╰╼═══╾╼═══╾╼═══╾")
                else:
